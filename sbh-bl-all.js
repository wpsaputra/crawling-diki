...
(async function main() {
  try {
    ...
    for (let i = 0; i < kabupaten.length; i++) {
      const new_kabupaten = kabupaten[i];
      
      const nks_response = await page.evaluate(async (cookies, triwulan, bulan, new_kabupaten) => {
          let response = await fetch("https://webapps.bps.go.id/olah/sbh2022/resource/wilayah/nks", {
              ...
              "body": "{\"kd_prov\":\"74\",\"kd_kab\":\""+new_kabupaten+"\",\"tw\":\""+triwulan+"\"}",
              "method": "POST",
              "mode": "cors",
              "credentials": "include"
          });
          ...
          // let nks = response;
          return nks;
      }, cookies, triwulan, bulan, new_kabupaten);
      ...
      for (let index = 0; index < nks_response.length; index++) {
        ...
        const entribl_response = await page.evaluate(async (cookies, nks, triwulan, bulan, new_kabupaten) => {
          let response = await fetch("https://webapps.bps.go.id/olah/sbh2022/resource/entriBL", {
              ...
              "referrer": "https://webapps.bps.go.id/olah/sbh2022/entriBL",
              "referrerPolicy": "strict-origin-when-cross-origin",
              "body": "{\"kd_prov\":\"74\",\"kd_kab\":\""+new_kabupaten+"\",\"nks\":"+nks+",\"bulan\":"+bulan+"}",
              "method": "POST",
              "mode": "cors",
              "credentials": "include"
          });
          ...
          // let nks = response;
          return entribl;
        }, cookies, nks, triwulan, bulan, new_kabupaten);
